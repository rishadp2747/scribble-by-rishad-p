...
class UserTest < ActiveSupport::TestCase
  ...
  def test_name_should_be_valid_with_valid_length
    @user.name = "a" * 30
    assert @user.valid?
  end
  ...
  def test_name_should_not_valid_with_invalid_length
    @user.name = "a" * 31
    assert_not @user.valid?
    assert_includes @user.errors.full_messages, "Name is too long (maximum is 30 characters)"
  end
  ...
  def test_email_should_be_valid_with_valid_length
    @user.email = "#{"a" * 41}@test.com"
    assert @user.valid?
  end

  def test_email_should_not_be_valid_with_invalid_length
    @user.email = "#{"a" * 42}@test.com"
    assert_not @user.valid?
    assert_includes @user.errors.full_messages, "Email is too long (maximum is 50 characters)"
  end
